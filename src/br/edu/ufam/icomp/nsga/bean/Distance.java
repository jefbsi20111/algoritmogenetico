...
import br.edu.ufam.icomp.nsga.comparator.ObjectiveComparator;
...
public class Distance {
	 ...
	 */
	public double[][] distanceMatrix(Populacao solutionSet) {
		Individuo solutionI, solutionJ;
	 ...
	 * @throws JMException
	 */
	public double distanceToSolutionSetInObjectiveSpace(Individuo solution,
			Populacao solutionSet) throws JMException {
	 ...
	 * @throws JMException
	 */
	public double distanceToSolutionSetInSolutionSpace(Individuo solution,
			Populacao solutionSet) throws JMException {
		// At start point the distance is the max
		double distance = Double.MAX_VALUE;
	 ...
	 * @throws JMException
	 */
	public double distanceBetweenSolutions(Individuo solutionI,
			Individuo solutionJ) throws JMException {
		/*
		...
		/*
		 * for (int i = 0; i < solI.getNumberOfDecisionVariables(); i++){ diff =
		 * solI.getValue(i) - solJ.getValue(i); distance += Math.pow(diff,2.0);
		 * }
		 */// for

		diff = solutionI.getAdequacao() - solutionJ.getAdequacao();
		distance += Math.pow(diff, 2.0);
		diff = solutionI.getCusto() - solutionJ.getCusto();
		distance += Math.pow(diff, 2.0);
	 ...
	 */
	public double distanceBetweenObjectives(Individuo solutionI,
			Individuo solutionJ) {
		double diff; // Auxiliar var
		double distance = 0.0;
		// -> Calculate the euclidean distance
	 ...
	 */
	public int indexToNearestSolutionInSolutionSpace(Individuo solution,
			Populacao solutionSet) {
		int index = -1;
	 ...
	 */
	public void crowdingDistanceAssignment(Populacao solutionSet, int nObjs) {
		...
		Populacao front = new Populacao(size);
		...
		for (int i = 0; i < nObjs; i++) {
			...
			front.ordenar(new ObjectiveComparator(i));
			if (i == 0) {
				objetiveMinn = front.get(0).getAdequacao();
				objetiveMaxn = front.get(front.size() - 1).getAdequacao();
			} else {
				objetiveMinn = front.get(0).getCusto();
				objetiveMaxn = front.get(front.size() - 1).getCusto();
			}
			...
			for (int j = 1; j < size - 1; j++) {
				if (i == 0) {
					distance = front.get(j + 1).getAdequacao()
							- front.get(j - 1).getAdequacao();
				}else{
					distance = front.get(j + 1).getCusto()
							- front.get(j - 1).getCusto();
